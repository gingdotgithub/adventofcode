...
while len(queue) > 0:
    ...
    if coord == target and steps > 3: #and steps >= 4: #this is if... its found the target. 
        endtime = time.time()
    ...
    if (coord,dir,steps) not in seen:
        ...
        if (steps < 10) and (dir != (0,0)): #assuming its trying to go in the same direction - can it still?
