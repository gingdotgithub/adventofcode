binary = ''
hexinput = list("")
#hexinput = list(input())
print(hexinput)
for hexi in hexinput:
    binary+=str(format(int(hexi,base=16), '04b'))
print(binary)

def read_metadata():
    global binary
    readlength = 0
    result = 0
    v,t,binary = binary[0:3],binary[3:6],binary[6:]
    readlength+=6
    print(v+"-"+t+"-"+binary)
    newpacket = [v,t,'',[]]
    if (int(t,2)) == 4:
        literallength,literal = read_literal()
        readlength+=literallength
        newpacket[2] = literal
        result = int(literal,2)
    else:
        operatorlength,subpackets, result = read_operator(int(t,2))
        readlength+=operatorlength
        newpacket[3] = subpackets
    return readlength, newpacket, result

def read_literal():
    global binary
    readlength = 0
    newliteral = ''
    while True:
        indicator,chunk, binary = binary[0:1],binary[1:5],binary[5:]
        newliteral+=chunk
        readlength+=5
        if indicator == '0':
            break
    return readlength, newliteral

def read_operator(operator):
    global binary
    results = []
    readlength = 1
    subpackets = []
    ltid,binary = binary[0:1], binary[1:]
    if ltid == '0':
        readlength+=15
        lengthbits,binary = binary[0:15],binary[15:]
        bitstoread = int(lengthbits,2)
        readlength+=bitstoread
        while bitstoread > 0:
            newpacketlen, newpacket, result = read_metadata()
            bitstoread-=newpacketlen
            subpackets.append(newpacket)
            results.append(result)
    else:
        readlength+=11
        packetcount,binary = binary[0:11],binary[11:]
        for n in range(int(packetcount,2)):
            newpacketlen, newpacket, result = read_metadata()
            subpackets.append(newpacket)
            readlength+=newpacketlen
            results.append(result)
    result = calc_result(operator,results)
    return readlength, subpackets, result


def calc_result(operator,values):
    result = 0
    if operator == 0:
        for x in values:
            result+=x
    elif operator == 1:
        result = 1
        for x in values:
            result = result*x
    elif operator == 2:
        result = values[0]
        for x in values:
            if x < result:
                result = x
    elif operator == 3:
        for x in values:
            if x > result:
                result = x
    elif operator == 5:
        if values[0] > values[1]:
            result = 1
    elif operator == 6:
        if values[0] < values[1]:
            result = 1
    elif operator == 7:
        if values[0] == values[1]:
            result = 1
    return result

readlngth,mainpacket,result = read_metadata()

print(mainpacket)
print(result)